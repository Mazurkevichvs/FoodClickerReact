        ...
        <div className="container">
          ...
          <section className="basket">
            <div className="summary__inner__container">
              <DeliveryMethod cafeName={'PENSJONAT LAGUNA'} postcode='80-808' adress='Uzdrowiskowa 12' openhours='8:00-21:00' deliveryOption={deliveryOption} setDeliveryOption={setDeliveryOption}/>
              <h1 className="basket__title">{deliveryOption === 'delivery' ? 'Dane do dostawy:' : 'Dane odbiorcy:'}</h1>
              <div className="basket__inner summary__inner">
                ...
                <Input label={'Nazwisko*:'} name="surname" id="surname" value={surname} required={true} onChange={(e: ChangeEvent<HTMLInputElement>) => setSurname(e.target.value)}/>
                { deliveryOption === 'delivery' && <><Input label={'Ulica*:'} name="street" id="street" value={street} required={true} onChange={(e: ChangeEvent<HTMLInputElement>) => setStreet(e.target.value)}/>
                ...
                <Input label={'Telefon*:'} name="phoneNumber" id="phoneNumber" value={phoneNumber} required={true} onChange={(e: ChangeEvent<HTMLInputElement>) => setPhoneNumber(e.target.value)}/>
                ...
                <textarea className="form__comment" name="message" id="comment" value={message} placeholder='np. piętro, kod do klatki, alergie' onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setMessage(e.target.value)}></textarea>
              </div>
              <h1 className="basket__title">Metoda płatności:</h1>
              <div className="basket__inner summary__inner summary__payment__method">
               <PaymentMethod name='Karta płatnicza' img='mastercard' isActive={paymentOption === 'Karta'} onClick={() => handleActivePaymentOption('Karta')}/>
               <PaymentMethod name='Apple pay' img='applepay' isActive={paymentOption === 'Apple'} onClick={() => handleActivePaymentOption('Apple')}/>
               <PaymentMethod name='BLIK' img='blik' isActive={paymentOption === 'Blik'} onClick={() => handleActivePaymentOption('Blik')}/>
               <PaymentMethod name='Gotówką na miejscu' img='cash' isActive={paymentOption === 'Gotowka'} onClick={() => handleActivePaymentOption('Gotowka')}/>
              </div>
            </div>
            <div className="summary__payment">
              ...
              <div className="summary__order__delivery">
                ...
                <p className="order__item__price">{deliveryOption === 'takeaway' ? '0zł' : '10zł'}</p>
              </div>
              <div className="summary__order__delivery">
                <h2 className="payment__title summary__payment__title ">Razem:</h2>
                <p className="order__item__price order__summary__price">{deliveryOption === 'takeaway' ? `${totalSum}zł` : `${totalSum + 10}zł`}</p>
              </div>
